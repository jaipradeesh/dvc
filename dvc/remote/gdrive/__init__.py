...
class RemoteGDrive(RemoteBASE):
    ...
    def get_metadata(self, path_info, fields=[]):
        parent = self.request(
    ...
    def makedirs(self, path_info):
        ...
        for path in parents(path_info):
            try:
                metadata = self.get_metadata(
                    self.path_cls.from_parts(
                        self.scheme, parent, path="/" + path.name
                    )
                )
                if not metadata_isdir(metadata):
                    raise GDriveError("{} is not a folder".format(path))
            except GDriveResourceNotFound:
                metadata = self.mkdir(parent, path.name)
            parent = metadata["id"]
        return parent
    ...
    def _upload(self, from_file, to_info, name, ctx, no_progress_bar):
        ...
        if not no_progress_bar:
            from_file = track_progress(name, from_file)
